...
async function fetchEventsFromRelay(url, filter) {
  try {
    ...
    const events = await pool.querySync([url], [{ ...filter }], { maxWait: 5000 });
  ...
  await Promise.all(relays.map(async (relayUrl) => {
    try {
      ...
      if (ack === true) {
        ...
        ok = ack.toLowerCase() === 'ok';
        if (!ok) {
          reason = ack;
        }
      } else if (Array.isArray(ack)) {
        const [, , accepted, message] = ack;
        ok = Boolean(typeof accepted === 'boolean' ? accepted : accepted === 'OK');
        if (!ok) {
          reason = message || 'Abgelehnt';
        }
      } else if (ack && typeof ack === 'object') {
        if ('ok' in ack) {
          ok = Boolean(ack.ok);
          if (!ok) {
            reason = ack.message || ack.reason || 'Abgelehnt';
          }
        } else {
          reason = JSON.stringify(ack);
        }
      } else if (typeof ack === 'number') {
        reason = ack.toString();
      } else {
        reason = 'Unbekannte Antwort';
      }
      if (ok) {
        updateRelayStatus(relayUrl, 'ok', 'Veroeffentlichung bestaetigt');
      } else {
        updateRelayStatus(relayUrl, 'error', reason || 'Veroeffentlichung fehlgeschlagen');
      }
      results.push({ relay: relayUrl, ok, reason });
    } catch (error) {
